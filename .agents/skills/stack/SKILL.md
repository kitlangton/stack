...
Works against GitHub (via the `gh` CLI), GitLab (via the `glab` CLI), and Azure
DevOps (via `az repos pr` with the `azure-devops` extension). Install and
authenticate the matching CLI before running `stack`. The `github.com`,
`gitlab.com`, and cloud Azure DevOps hosts (`dev.azure.com`, `ssh.dev.azure.com`,
...
PR references as `#123`; GitLab and Azure DevOps blocks use `!123` (`#` in ADO
descriptions links to work items). GitLab blocks also render scannable MR entries
with titles, for example `!123 - Add parser`. Branch paths are intentionally omitted from
stack blocks.
