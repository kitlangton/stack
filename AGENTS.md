...
- Keep local Git behavior behind `Git` and pull/merge-request behavior behind `CodeHost`. Concrete backends live in `services/code-host/GitHub.ts` (via `gh`), `services/code-host/GitLab.ts` (via `glab`), and `services/code-host/AzureDevOps.ts` (via `az repos pr`), while their in-memory contract behavior is shared through `services/code-host/Memory.ts`; the CLI picks one backend at startup from `STACK_CODE_HOST`, `git config stack.codeHost`, or an unambiguous `origin` host (`github.com`, `gitlab.com`, or cloud Azure DevOps). Stack orchestration depends on `CodeHost.Service` rather than shelling out to a host CLI directly.
