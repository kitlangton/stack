...
- **Code host**: The hosted platform containing pull or merge requests. Currently GitHub (via `gh`), GitLab (via `glab`), and Azure DevOps (via `az repos pr`). Known public hosts are selected from `origin`; enterprise or on-prem hosts are configured with `git config stack.codeHost`.

## Architecture Notes

- Local Git behavior stays behind the `Git` seam.
- Pull/merge-request behavior stays behind the `CodeHost` seam; its provider-neutral `changes()` and `change()` operations map to GitHub PRs, GitLab MRs, or Azure DevOps pull requests in concrete backends under `services/code-host/`.
